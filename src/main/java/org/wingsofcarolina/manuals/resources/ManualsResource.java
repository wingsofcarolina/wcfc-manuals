...
public class ManualsResource {
  ...
  private void loadDataStore()
    ...
    try {
      ...
      aircraftCache =
        Arrays.asList(
          mapper.readValue(
            Paths.get(root + "/" + AIRCRAFT_JSON).toFile(),
            Aircraft[].class
          )
        );
      ...
      equipmentCache =
        Arrays.asList(
          mapper.readValue(
            Paths.get(root + "/" + EQUIPMENT_JSON).toFile(),
            Equipment[].class
          )
        );
    } finally {
  ...
  @Path("reload")
  @Produces(MediaType.APPLICATION_JSON)
  public Response reload(@CookieParam("wcfc.manuals.token") Cookie cookie)
    throws JsonParseException, JsonMappingException, IOException {
  ...
  private void writeJson(String name, List entities) {
    try {
      ...
      try (
        FileOutputStream fos = new FileOutputStream(root + "/" + name + ".json");
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public Response registerEquipment(
    @CookieParam("wcfc.manuals.token") Cookie cookie,
    Map<String, String> request
  ) {
    User user = authUtils.getUserFromCookie(cookie);
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public Response registerAircraft(
    @CookieParam("wcfc.manuals.token") Cookie cookie,
    Map<String, String> request
  ) {
    User user = authUtils.getUserFromCookie(cookie);
      ...
      if (newfile.exists()) {
        ...
        if ((aircraft = updateAircraftStore(identifier)) != null) {
          ...
          Slack
            .instance()
            .sendMessage(
              Slack.Channel.MANUALS,
              uploadMessage(user, ManualType.AIRCRAFT, aircraft.getRegistration())
            );
        }
        if ((equipment = updateEquipmentStore(identifier)) != null) {
          ...
          Slack
            ...
            .sendMessage(
              Slack.Channel.MANUALS,
              uploadMessage(user, ManualType.EQUIPMENT, equipment.getName())
            );
        }
  ...
  private String uploadMessage(User user, ManualType type, String identifier) {
    ZoneId zoneId = ZoneId.of("US/Eastern");
    ZonedDateTime now = LocalDateTime.now().atZone(zoneId);
    ...
    StringBuilder msg = new StringBuilder();
    msg
      .append("*WCFC Manuals Server notification sent at : ")
      .append(dateFormatGmt.format(now))
      .append("*\n");
    msg.append("**New Manual Uploaded**\n");
    msg.append("**Uploaded by:** ").append(user.getName()).append("\n");
    msg.append(message);
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public Response archiveDetails(@CookieParam("wcfc.manuals.token") Cookie cookie)
    throws IOException {
  ...
  public Response archive(@CookieParam("wcfc.manuals.token") Cookie cookie)
    ...
    if (user != null) {
      ...
      Slack
        ...
        .sendMessage(
          Slack.Channel.MANUALS,
          archiveMessage(user, filename, humanReadableByteCountBin(bytes))
        );
  ...
  private String archiveMessage(User user, String filename, String size) {
    ZoneId zoneId = ZoneId.of("US/Eastern");
    ZonedDateTime now = LocalDateTime.now().atZone(zoneId);
    ...
    msg
      ...
      .append(dateFormatGmt.format(now))
      .append("*\n");
    msg.append("**Archive Updated**\n");
    msg.append("**Updated by:** ").append(user.getName()).append("\n");
    msg
      .append("A new WCFC Manuals ZIP archive has been generated by ")
      .append(user.getName());
    msg
      .append(". The archive name is '")
      .append(filename)
      .append("' and it is ")
      .append(size)
      .append(" in size.");
  ...
  private String zipOutputName(String uuid) {
    ...
    if (zipDirectory != null && zipName != null) {
      return (
        zipDirectory.replace(' ', '_').replace('/', '_') +
        "/" +
        zipName.replace(' ', '_').replace('/', '-').replace('\'', '^') +
        ".pdf"
      );
    } else {
  ...
  private StringBuffer generateGuidePage(String filename) {
    ...
    sb.append(
      "<div class='container'><div class='header'><img src='img/WCFC-logo.jpg'>\n"
    );
    sb.append("<span style='vertical-align: top'>WCFC Flight Manuals</span></div>\n");
    sb.append(
      "<div class='header'><span style='font-size: 0.6em;'>Source : " +
      filename +
      "</span></div><hr>"
    );
    ...
    for (Aircraft acft : aircraftCache) {
      sb.append(
        "<th class='reg'><a href='data/" +
        zipOutputName(acft.getUuid()) +
        "' target=_blank>" +
        acft.getRegistration() +
        "</a></th>\n"
      );
    }
    sb.append("</tr>");
    ...
    for (Map type : EquipmentType.getTypes()) {
      ...
      for (Equipment e : equipmentCache) {
        if (e.getType().equals(t)) {
          sb.append(
            "<tr class='detail'><td class='equipment'><a href='data/" +
            zipOutputName(e.getUuid()) +
            "' target=_blank>" +
            e.getName() +
            "</a></td>"
          );
          ...
          for (Aircraft acft : aircraftCache) {
            if (equipmentInstalled(acft, e)) sb.append(
              "<td><img src='img/checkmark.png' alt='X'></td>"
            ); else sb.append("<td><img src='img/unchecked.png' alt='-'></td>");
          }
          sb.append("</tr>");
        }
      }
  ...
  private boolean equipmentInstalled(Aircraft acft, Equipment e) {
    ...
    for (String eid : eidList) {
      if (eid.equals(e.getUuid())) return true;
    }
    return false;
  }
